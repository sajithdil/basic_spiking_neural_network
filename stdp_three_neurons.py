...
#according to this dont ever set weights to 0: https://github.com/gaiasilvestri/spike-time-dependent-plasticity/blob/master/stdp
#when 0 even though it fires the weight never changes, so it looks like no synaptic connection exists
#the above uses 0 on the weight matrix (to remove synaptic connections to make the network sparse)

v1v3W = -0.1
v2v3W = 0.1

v1v3W_changes = np.full(tmax,-0.1)
v2v3W_changes = np.full(tmax,0.1)
